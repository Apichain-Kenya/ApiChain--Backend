"""Unit tests for `_verify_process_hash` — Sprint 5 process stage."""

from types import SimpleNamespace

from app.routers.batch import _verify_process_hash, _process_record_canonical_payload
...
def _row(**overrides) -> SimpleNamespace:
    base = dict(
        ...
        moisture_content=17.8,
        handling_notes="filtered through 200-micron mesh",
        process_proof_hash=None,
    )
...
def test_three_way_match_when_chain_db_and_recomputed_agree():
    row = _row()
    h = _hash_of(row)
    row.process_proof_hash = h

    result = _verify_process_hash(row, h)
...
def test_tampering_after_persisted_hash_breaks_match():
    ...
    row.process_proof_hash = persisted_hash

    row.extraction_method = "centrifuge"

    result = _verify_process_hash(row, persisted_hash)
...
def test_zero_chain_hash_never_matches():
    ...
    zero = "0x" + ("00" * 32)
    result = _verify_process_hash(row, zero)
